        ...
        <div class="pa-4">
          ...
          <form v-on:submit.prevent="createTShirtSizeInternal()">
            ...
            <v-row class="v-row" justify="end">
              <v-btn color="primary" :loading="loadingTShirtSize !== '' && loadingTShirtSize === newTShirtSize" type="submit" rounded>
                <span>Erstellen</span>
              </v-btn>
            </v-row>
          </form>
        ...
        <div class="px-4">
          ...
          <div class="flex-row flex-center">
            <v-card class="event-card mt-6 p-6">
              ...
              <div class="flex-row event" v-for="tShirtSize in tShirtSizes" :key="tShirtSize.size">
                <div class="flex-row flex-grow">
                  ...
                  <div>
                    <v-icon
                      medium
                      @click.prevent="openDeleteDialog(tShirtSize)"
                    >
                      mdi-delete
                    </v-icon>
                  </div>
    ...
    <v-card class="mb-0">
      <form v-on:submit.prevent="deleteEventInternal">
        ...
        <v-card-text v-if="tShirtSizeToDelete === tShirtSizeToReplace">
          <v-icon medium>mdi-warning</v-icon> Die Größe kann nicht durch die gleiche Größe ersetzt werden.
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn rounded @click="closeDeleteDialog"> Schließen </v-btn>
          <v-btn color="primary"  :loading="deletingRequest" type="submit" :disabled="tShirtSizeToReplace === '' || tShirtSizeToReplace === tShirtSizeToDelete">
            Ersetzen
          </v-btn>
        </v-card-actions>
      </form>
